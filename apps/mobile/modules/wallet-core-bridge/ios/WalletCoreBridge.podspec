Pod::Spec.new do |s|
  ...
  # Stage 5D.6B: durable, tracked unit-test target for native-only (non-Expo)
  # Swift code such as WalletSecureStorage.swift. Lives in the podspec itself
  # (not apps/mobile/ios/, which is fully gitignored/regenerated by `expo
  # prebuild` and would silently discard a hand-added Xcode test target) so
  # `pod install` regenerates the test target correctly every time.
  # A host app is required for XCTest execution on a physical iOS device.
  s.test_spec 'Tests' do |test_spec|
    test_spec.test_type = :unit
    test_spec.requires_app_host = true
    test_spec.source_files = 'Tests/*.swift'
    # The standalone .xctest bundle transitively links C++ code from React
    # Native's own generated codegen libraries (via the ExpoModulesCore ->
    # React Native dependency chain) without automatically linking libc++,
    # unlike the main app target. Scoped to this test subspec only — the
    # main WalletCoreBridge target's own xcconfig is untouched.
