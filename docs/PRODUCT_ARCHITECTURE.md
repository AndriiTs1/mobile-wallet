...
- **Dead code:** `mockPortfolio` (the whole object) is defined but never imported anywhere else in the app (verified via `grep`). `MockAsset.valueLabel`, `.changeLabel`, and `.isPositive` are defined and populated with stale hand-typed values but are never read anywhere — both Home and Assets compute their own live values and pass them explicitly instead. These should be removed in a future cleanup pass rather than left to imply they're load-bearing.
